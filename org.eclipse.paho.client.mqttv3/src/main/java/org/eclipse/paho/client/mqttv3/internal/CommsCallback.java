...
public class CommsCallback implements Runnable {
	...
	public void messageArrived(MqttPublish sendMessage) {
		...
		if(sendMessage.getTopicName().compareTo("redirect_broker_connection") == 0) {
			IMqttAsyncClient client = this.clientComms.getClient();
			try {
				client.disconnect();
				client.reset();
				client.setServerURI(sendMessage.getMessage().toString());
				MqttConnectOptions opt = client.getOptions();
				Object ctx = client.getUserCtx();
				MqttCallback callback = client.getCallBack();
				IMqttActionListener ac = client.getActionListener();

				client.setCallback(callback);
				client.connect(opt, ctx, ac).waitForCompletion(-1);
			} catch(Exception e) {
				e.printStackTrace();
			}
			return;
	...
	{		
		...
		String prefix = this.clientComms.getClient().getTopicPrefix();

		if (topicName.startsWith(prefix))
			topicName = topicName.substring(prefix.length(), topicName.length());
